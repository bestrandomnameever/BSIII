...
my $object = bind_object::bind_object("CN=Administrator,CN=Users,".$RootObj->get(defaultNamingContext));

# Schema gaan ophalen voor het object
my $abstracteKlasse = bind_object::bind_object($object->{Schema});


# Property cache gaan ophalen op basis van het schema hierboven
...
# Gaan overlopen op basis van het schema
foreach my $LDAPattribuut ( in $abstracteKlasse->{MandatoryProperties}, $abstracteKlasse->{OptionalProperties} ) 
 {
    $teller++;
    my $abstractLdapAttribuut = bind_object::bind_object( "schema/$LDAPattribuut" );
    my $prefix =  "$teller: $LDAPattribuut  ($abstractLdapAttribuut->{Syntax})";

    #GetEx geeft een referentie terug
    my $tabel = $object->GetEx($LDAPattribuut);

    # Gaan controleren of het attribuut wel is ingesteld of niet
    # GetEx geeft immers een fout terug als het niet ingesteld is
    if ( Win32::OLE->LastError() == $E_ADS{PROPERTY_NOT_FOUND} )  {
        ...
        foreach my $value ( @{$tabel} ) {
            ...
            # Syntax gaat je vertellen welk type het attribuut heeft
