# implementatie bind_object::bind_object functie: zie sectie 5
...
my $cont = bind_object::bind_object("OU=EM7INF,OU=Studenten,OU=iii,".$RootObj->get(defaultNamingContext));
my $user=$cont->getObject("user","cn=$wie");

unless ($user) {
    $cont = bind_object::bind_object("OU=Docenten,OU=iii,".$RootObj->get(defaultNamingContext));
...
sub isset{
   ...
   $user->GetInfoEx([$property],0);     #vraag expliciet om property in de cache te plaatsen
    @array = (grep {lc($user->Item($_)->{Name}) eq lc($property)} 0..$user->{PropertyCount}-1);
    print Dumper \@array;
